...
#ifdef CONFIG_MAGIC_LED
#include <linux/timer.h>
#include <linux/timex.h>

extern int magic_led_id;
...
void remote_keydown(struct remote_dev *dev, int scancode, int status)
{
	...
	if (dev->keypressed) {
		...
		//printk("----CONFIG_MAGIC_LED keypressed scancode=:%d \n",scancode);
				//if (scancode == 0xf00ffe01){
						if (scancode == 15){
						
                     /*  11 0x7c837f80  33 0xb34cfe01   app 0xf00ffe01*/
					get_monotonic_boottime(&uptime);
					boottime = (unsigned long)uptime.tv_sec;
					if ((boottime <= 16)|| (issuspend == 1)){
						//break;
